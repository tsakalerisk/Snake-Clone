...
enum { HEAD, BODY_YELLOW, BODY_BLACK, CORNER_YELLOW, CORNER_BLACK, TAIL_BLACK, TAIL_YELLOW};
...
void LinkedInsert(ListPointer *List, coord Item, ListPointer PredPtr, ListPointer *LastPtr);
void AdvanceList(ListPointer List, coord newTile);
bool Search(ListPointer List, coord item);
void SelectTile(ListPointer CurrPtr, ListPointer worm_head, ListPointer worm_tail, SDL_Rect *tile, double *rotation, int i, char looking);
void DeleteTail(ListPointer *worm_tail);
void ResetGame(ListPointer *worm_tail, char *looking, ListPointer *worm_head, coord *fruit);
...
void Move(char* looking, ListPointer* worm_tail, ListPointer* worm_head, coord* fruit);
void Update(char looking, ListPointer worm_tail, ListPointer worm_head, coord fruit);
void AddQ(QueueType* Queue, SDL_Event Item);
int RemoveQ(QueueType* Queue, SDL_Event* Item);
void InitGame(char* looking, ListPointer* worm_tail, ListPointer* worm_head, coord* fruit);
void MainMenu();
void PauseMenu();
void SkinMenu();
void RenderMenu(SDL_Rect menu_rect, int nOptions, std::string* options, int selection);
...
const SDL_Rect gInfoRect = { 0, GAME_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT - GAME_HEIGHT };
...
std::string gPause_Options[N_PAUSE_OPTIONS] = { "CONTINUE", "CHANGE  SKIN", "QUIT" };
				...
				if (curr_time - start_time > 130) {
					...
					while (RemoveQ(&KeyPressQ, &key_press) != 0) {
						...
						case SDLK_UP:
							...
							break;
						case SDLK_DOWN:
							if (looking != DOWN && looking != UP && looking != DEFAULT) {
								looking = DOWN;
								Move(&looking, &worm_tail, &worm_head, &fruit);
								moved = true;
							}
...
long GetHighScore() {
	FILE *highscore_file;
	fopen_s(&highscore_file, "highscore.worm", "rb");
	long highscore = 0;
...
void SetHighScore(long highscore) {
	FILE *highscore_file;
	fopen_s(&highscore_file, "highscore.worm", "wb");
	if (highscore_file != NULL) {
...
void ResetGame(ListPointer *worm_tail, char *looking, ListPointer *worm_head, coord *fruit) {
	...
	*worm_head = NULL;
	for (int i = 0; i < 3; i++) {
		LinkedInsert(worm_tail, { SCREEN_WIDTH / 2, GAME_HEIGHT / 2 + (i - 1)* 20}, NULL, worm_head);
	}
	*looking = DEFAULT;
...
void SelectTile(ListPointer CurrPtr, ListPointer worm_head, ListPointer worm_tail, SDL_Rect *tile, double *rotation, int i, char looking) {
	if (CurrPtr == worm_head) {
		...
		if (next.x == curr.x) { //up or down
			if (next.y == curr.y - 20) *rotation = 0;
			else if (next.y == curr.y + 20) *rotation = 180;
		}
		else if (next.y == curr.y) { //left or right
			if (next.x == curr.x - 20) *rotation = -90;
			else if (next.x == curr.x + 20) *rotation = 90;
		}
	}
	else {
		...
		if (pred.x == next.x) { //vertical line
			...
			*rotation = 90;
			...
			if (pred.y == curr.y - 20 || next.y == curr.y - 20) { //J or L
				...
				if (pred.x == curr.x - 20 || next.x == curr.x - 20) { //mirror gamma
					*rotation = -90;
					if (i % 2 == 0) {
						tile->x = CORNER_BLACK;
					}
...
bool init() {
	...
	if (SDL_Init(SDL_INIT_VIDEO) < 0) {
		...
		gWindow = SDL_CreateWindow("Worm", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (gWindow == NULL) {
...
SDL_Texture* loadTexture(std::string path) {
	...
	if (loadedSurface == NULL) printf("Unable to load image %s! SDL_Error: %s\n", path.c_str(), IMG_GetError());
		...
		if (newTexture == NULL) printf("Unable to create texture from %s! SDL_Error: %s\n", path.c_str(), SDL_GetError());
...
void Move(char* looking, ListPointer* worm_tail, ListPointer* worm_head, coord* fruit) {
	...
	if (moved) {
		if (Search(*worm_tail, new_head)) {
			...
			if ((*worm_head)->Data.x == fruit->x && (*worm_head)->Data.y == fruit->y) {
				LinkedInsert(worm_tail, (*worm_tail)->Data, NULL, worm_head);
				gScore += 5;
				do {
					fruit->x = rand() % (SCREEN_WIDTH / 20) * 20;
...
void Update(char looking, ListPointer worm_tail, ListPointer worm_head, coord fruit) {
	...
	while (CurrPtr != NULL) {
		...
		tile.x *= 20;
		SDL_RenderCopyEx(gRenderer, gTextureWorm, &tile, &position, rotation, NULL, SDL_FLIP_NONE);;
		...
		while (SDL_PollEvent(&e) != 0) {
			if (e.type == SDL_QUIT) {
				...
				case SDLK_RETURN: //enter
					...
					{
						SDL_Surface* win_surface = SDL_GetWindowSurface(gWindow), * temp_surface = SDL_CreateRGBSurface(0, SCREEN_WIDTH, SCREEN_HEIGHT, 32, 0, 0, 0, 0);
						SDL_RenderReadPixels(gRenderer, NULL, 0, temp_surface->pixels, temp_surface->pitch);
						SDL_Texture* temp_texture = SDL_CreateTextureFromSurface(gRenderer, temp_surface);
						SkinMenu();
		...
		while (SDL_PollEvent(&e) != 0) {
			if (e.type == SDL_QUIT) {
				...
				case SDLK_DOWN:
					...
					break;
				case SDLK_LEFT:
					if (selection == SKIN_NUMBER && SkinExists(skin_previewed - 1)) {
...
void RenderMenu(SDL_Rect menu_rect, int nOptions, std::string *options, int selection) {
	...
	for (int i = 0; i < nOptions; i++) {
		...
		if (i == selection) {
			...
			SDL_RenderDrawLine(gRenderer, menu_rect.x, text_rect.y, menu_rect.x + menu_rect.w, text_rect.y);
			SDL_RenderDrawLine(gRenderer, menu_rect.x, text_rect.y + text_rect.h, menu_rect.x + menu_rect.w, text_rect.y + text_rect.h);
		}
	}
...
void RenderSkinPreview(SDL_Rect preview_rect, int skin_previewed) {
	...
	int parts[] = { TAIL_BLACK, BODY_YELLOW, BODY_BLACK, BODY_YELLOW, BODY_BLACK, HEAD };

	for (int i = 0; i < 6; i++) {
		SDL_Rect src = { parts[i] * 20, 0, 20, 20 };
		SDL_Rect dst = { (preview_rect.x + preview_rect.w - 6 * 20) / 2 + i * 20, preview_rect.y + ( preview_rect.h - 20) / 2, 20, 20 };
		SDL_RenderCopyEx(gRenderer, preview_texture, &src, &dst, 90, NULL, SDL_FLIP_NONE);
	}

	SDL_Texture* arrow_texture = loadTexture(ResourcesSlash("arrow.png"));
	if (SkinExists(skin_previewed - 1)) {
		SDL_Rect dst = { (preview_rect.x + preview_rect.w - 30) / 2 - 100, preview_rect.y + (preview_rect.h - 30) / 2, 30, 30 };
		SDL_RenderCopy(gRenderer, arrow_texture, NULL, &dst);
	}
	if (SkinExists(skin_previewed + 1)) {
		SDL_Rect dst = { (preview_rect.x + preview_rect.w - 30) / 2 + 100, preview_rect.y + (preview_rect.h - 30) / 2, 30, 30 };
		SDL_RenderCopyEx(gRenderer, arrow_texture, NULL, &dst, 0, NULL, SDL_FLIP_HORIZONTAL);
	}
...
bool SkinExists(int n) {
	...
	path.append(".png");
	FILE* temp_file;
	fopen_s(&temp_file, path.c_str(), "r");
	if (temp_file == NULL) {
		return false;
	}
	else {
