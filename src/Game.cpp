...
Game::Game(const std::string& name, const int width, const int height)
    ...
    if (!initSDL(name, width, height)) printf("SDL initialization error!");
...
bool Game::initSDL(const std::string& name, const int width, const int height) {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        printf("SDL could not initialiaze! SDL_Error: %s\n", SDL_GetError());
        return false;
    }

    gWindow = SDL_CreateWindow(name.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width,
    ...
    // a hardware accelerated renderer is preferred by default
